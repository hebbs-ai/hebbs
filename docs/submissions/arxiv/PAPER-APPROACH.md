...
The comparison table. Not a related work dump, but a pointed gap analysis. One table showing the full pipeline: encode, associate, detect, resolve, consolidate, decay. Checkmarks for HEBBS. Partial marks for everyone else. Nobody fills the row.
...
Limitations honestly. Future work: multi-agent shared memory via `.hebbs/` sharing, cross-vault consolidation. The "testable predictions" from the neuroscience mapping, specifically what the analogy predicts that we haven't built yet.
